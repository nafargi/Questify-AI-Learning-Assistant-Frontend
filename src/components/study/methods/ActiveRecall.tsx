...
import { Brain, Eye, ArrowCounterClockwise, Check, Sparkle, X, Target, ArrowRight, Lightbulb, BookOpen, ArrowLeft, CircleNotch } from "@phosphor-icons/react";
...
export function ActiveRecall({ onBack, collectionId, studyData, topic }: {
    onBack: () => void;
    collectionId?: string;
    studyData: any;
    topic?: string;
    ...
    return (
        <StudySessionLayout
            title="Active Recall"
            subtitle={localTopic || topic || "Self-Testing Protocol"}
            icon={Brain}
            color="text-rose-500"
            onExit={onBack}
            rightAction={
            ...
            <div className="flex h-full overflow-hidden bg-background">
                ...
                <div className="flex-1 flex flex-col relative min-w-0">
                    ...
                    <ScrollArea className="flex-1 p-8">
                        <div className="max-w-xl mx-auto flex flex-col justify-center gap-8 py-10 min-h-[60vh]">
                            {prompts.length === 0 ? (
                                ...
                                <>
                                    {/* Question */}
                                    <Card className="p-10 shadow-2xl border-none rounded-[2.5rem] overflow-hidden relative bg-white ring-1 ring-black/5">
                                    ...
                                    {currentQ?.hint && (
                                        <div className="flex justify-center">
                                            {!isHintVisible ? (
                                                ...
                                                <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-sm text-amber-800 flex items-start gap-2 animate-in fade-in duration-300 w-full">
                                    ...
                                    {!isAnswerVisible ? (
                                        <button
                                            ...
                                            className="w-full h-[180px] border-4 border-dashed border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 text-slate-300 hover:bg-rose-500/5 hover:border-rose-500/30 hover:text-rose-500 transition-all duration-500 group"
                                        >
                                        ...
                                        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                                            ...
                                            <div className="flex gap-3">
                                                <Button variant="outline" size="lg"
                                                    className="flex-1 h-14 rounded-2xl border-rose-200 text-rose-700 font-black uppercase tracking-widest text-[10px] hover:bg-rose-50"
                                                    onClick={() => handleNext(false)}>
                                                    <X className="w-5 h-5 mr-2" weight="bold" /> Needs Practice
                                                </Button>
                                                <Button size="lg"
                                                    className="flex-1 h-14 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-black uppercase tracking-widest text-[10px] shadow-xl shadow-rose-600/20"
                                ...
                                <div className="text-center space-y-8 animate-in zoom-in duration-500">
                                    <div className="w-20 h-20 bg-rose-600 text-white rounded-[2rem] flex items-center justify-center mx-auto shadow-2xl shadow-rose-600/30">
                                        <Sparkle className="w-10 h-10" weight="bold" />
                                    </div>
                                    <h2 className="text-3xl font-black">Recall Complete!</h2>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-card p-5 rounded-2xl border">
                                            <div className="text-2xl font-black text-rose-600">{accuracy}%</div>
                                            <div className="text-[9px] uppercase font-bold text-muted-foreground mt-1">Accuracy</div>
                                        </div>
                                        <div className="bg-card p-5 rounded-2xl border">
                                            <div className="text-2xl font-black text-green-600">{score.correct}</div>
                                            <div className="text-[9px] uppercase font-bold text-muted-foreground mt-1">Correct</div>
                                        </div>
                                    </div>
                                    <Button size="lg" onClick={() => { setCurrentIndex(0); setScore({ correct: 0, total: 0 }); }} className="rounded-2xl px-10 h-14 font-bold">
                                        <ArrowCounterClockwise className="mr-2" /> Try Again
                                    </Button>
                                </div>
                            )}
                        </div>
